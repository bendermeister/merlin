...
BENCH_findallpcome128_find(const mrln_str8view_t haystack[static 1],
...

  {
    ...
    const mrln_v16u8_unaligned_t *first_buffer =
        (const mrln_v16u8_unaligned_t *)haystack->buffer;
    const mrln_v16u8_unaligned_t *last_buffer =
        (const mrln_v16u8_unaligned_t *)(haystack->buffer + last_char_offset);
    mrln_v16u8_t row_in_f;
    mrln_v16u8_t row_in_l;
    intptr_t i = 0;
    intptr_t mask;
    for (; i < v16u8_iter_end;) {
      ...
      for (; i < v16u8_iter_end && result.length < result_end; ++i) {
        ...
        row_in_f = mrln_v16u8_load_unaligned(first_buffer + i);
        row_in_f = mrln_v16u8_cmpeq(row_in_f, row_first_char);

        row_in_l = mrln_v16u8_load_unaligned(last_buffer + i);
        row_in_l = mrln_v16u8_cmpeq(row_in_l, row_last_char);

        mask = mrln_v16u8_mask(mrln_v16u8_and(row_in_f, row_in_l));
...
static int
BENCH_findallpcome128_replace(mrln_str8_t s[static 1],
...
BENCH_findallpcome128_bench(mrln_str8view_t target[static 1],
