...
static void row32_search(const merlin_v8u32_unaligned_t addr[static 1],
                         BENCH_findallpcome3_result_t result[static 1],
                         intptr_t index, intptr_t offset,
                         merlin_v8u32_t row_selection_mask,
                         merlin_v8u32_t row_key) {
  merlin_v8u32_t row;
  row = merlin_v8u32_load_unaligned(addr);
  row = merlin_v8u32_and(row, row_selection_mask);
  row = merlin_v8u32_cmpeq(row, row_key);
  int mask = merlin_v8u32_mask(row);
  int index_offset = 0;
  while (mask) {
...
BENCH_findallpcome3_find(const merlin_str8_view_t haystack[static 1],
  ...
  {
    intptr_t index = 0;
    const merlin_v8u32_t row_key = merlin_v8u32_set1(key);
    const merlin_v8u32_t row_selection_mask = merlin_v8u32_set1(0xffffffu);
    merlin_v8u32_t row;
    merlin_v8u32_t row_data;
    uint32_t mask;
    int index_offset = 0;
    ...
    for (; index < v8u32_end;) {
      ...
      for (; index < v8u32_end && result.length < result_end; index += 32) {
        row_data =
            merlin_v8u32_load_unaligned((void *)(haystack->buffer + index));
        row = merlin_v8u32_and(row_data, row_selection_mask);
        row = merlin_v8u32_cmpeq(row, row_key);
        mask = merlin_v8u32_mask(row);
        index_offset = 0;
        while (mask) {
          const int ctz = __builtin_ctz(mask);
          index_offset += ctz;
  ...
  if (index + 16 + 4 < haystack->length) {
    const merlin_v4u32_t row_key = merlin_v4u32_set1(key);
    const merlin_v4u32_t row_selection_mask = merlin_v4u32_set1(0xffffffu);
    merlin_v4u32_t row;
    uint32_t mask;
    row = merlin_v4u32_load_unaligned((void *)(haystack->buffer + index));
    row = merlin_v4u32_and(row, row_selection_mask);
    row = merlin_v4u32_cmpeq(row, row_key);
    mask = merlin_v4u32_mask(row);
    {
      int index_offset = 0;
      while (mask) {
        const int ctz = __builtin_ctz(mask);
    ...
    row = merlin_v4u32_load_unaligned((void *)(haystack->buffer + index + 1));
    row = merlin_v4u32_and(row, row_selection_mask);
    row = merlin_v4u32_cmpeq(row, row_key);
    mask = merlin_v4u32_mask(row);
    {
    ...
    row = merlin_v4u32_load_unaligned((void *)(haystack->buffer + index + 2));
    row = merlin_v4u32_and(row, row_selection_mask);
    row = merlin_v4u32_cmpeq(row, row_key);
    mask = merlin_v4u32_mask(row);
    {
    ...
    row = merlin_v4u32_load_unaligned((void *)(haystack->buffer + index + 3));
    row = merlin_v4u32_and(row, row_selection_mask);
    row = merlin_v4u32_cmpeq(row, row_key);
    mask = merlin_v4u32_mask(row);
    {
...
static int
BENCH_findallpcome3_replace(merlin_str8_t s[static 1],
...
BENCH_findallpcome3_bench(merlin_str8_view_t target[static 1],
