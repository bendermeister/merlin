...
static void BM_insert_fixlen_bl(benchmark::State &state) {
  ...
  for (auto _ : state) {
    mrln_smap_t map = {};
    (void)mrln_smap(&map, num_elements, a);
    for (intptr_t i = 0; i < num_elements; ++i) {
      auto x = data.key[i];
      benchmark::DoNotOptimize(x);
    }
    mrln_smap_destroy(&map, a);
  }
}
...
static void BM_insert_fixlen(benchmark::State &state) {
  ...
  for (auto _ : state) {
    ...
    (void)mrln_smap(&map, num_elements, a);
    for (intptr_t i = 0; i < num_elements; ++i) {
      (void)mrln_smap_insert(&map, data.key[i], (uintptr_t)i, a);
    }
    mrln_smap_destroy(&map, a);
  }
}
...
static void BM_find_fixlen(benchmark::State &state) {
  ...
  for (auto _ : state) {
    auto i = mrln_smap_find(&map, data.key[rand() % num_elements]);
